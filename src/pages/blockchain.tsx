  ...
  return (
    <div className="flex flex-col gap-6 animate-in fade-in duration-700">
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
        <Card className="bg-white border-slate-200 shadow-md overflow-hidden group">
          <CardContent className="p-6 relative">
            ...
            <h3 className="text-3xl font-black tabular-nums text-slate-800">
        ...
        <Card className="bg-white border-slate-200 shadow-md group">
          <CardContent className="p-6 relative">
            <div className="absolute top-0 right-0 p-4 opacity-10 group-hover:scale-110 transition-transform">
        ...
        <Card className="bg-white border-slate-200 shadow-md group">
          <CardContent className="p-6 relative">
            ...
            <div className="flex items-center gap-2 mt-1">
              ...
              <h3 className="text-lg font-bold uppercase tracking-tight text-slate-800">{networkStatus?.status || 'Online'}</h3>
            </div>
            <p className="text-[10px] text-emerald-500/60 mt-1 font-medium italic">{networkStatus?.node_count || 12} decentralized nodes</p>
          </CardContent>
        </Card>

        <Card className="bg-primary text-white border-none shadow-lg relative overflow-hidden flex flex-col justify-center items-center group cursor-pointer transition-all hover:scale-105" onClick={handleVerify}>
          ...
          {verificationResult && (
            <div className="absolute bottom-2 px-3 py-1 bg-white/30 rounded-full backdrop-blur-md text-[9px] font-bold border border-white/20 animate-in slide-in-from-bottom-2">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <Card className="lg:col-span-2 border-slate-200 shadow-md bg-white overflow-hidden">
          <CardHeader className="flex flex-row items-center justify-between border-b border-slate-100 bg-slate-50/50 py-4">
            <div>
              <CardTitle className="text-xl font-bold tracking-tight text-slate-800">Evidence Registry</CardTitle>
              <CardDescription className="text-slate-500">Immutable record of all captured files and forensics</CardDescription>
            </div>
            <div className="flex gap-2">
              <Button variant="ghost" size="icon" className="h-8 w-8 hover:bg-slate-100 text-slate-500" disabled={currentPage === 1} onClick={() => setCurrentPage(p => p - 1)}>
                <ChevronLeft className="h-4 w-4" />
              </Button>
              <Button variant="ghost" size="icon" className="h-8 w-8 hover:bg-slate-100 text-slate-500" onClick={() => setCurrentPage(p => p + 1)}>
              ...
              <TableBody>
                {evidenceRecords.length === 0 ? (
                  ...
                  evidenceRecords.map((record) => (
                    <TableRow key={record.evidence_id} className="border-slate-100 group hover:bg-slate-50/50 transition-colors">
                      ...
                      <TableCell>
                        <div className="font-mono text-xs text-slate-700">{record.filename}</div>
                      ...
                      <TableCell>
                        <div className="flex flex-col gap-1 items-start">
                          ...
                          <div className="flex items-center gap-1 text-[10px] text-slate-500">
                            <ShieldCheck className="h-3 w-3 text-slate-400" /> {record.mitre_technique}
                          </div>
                        </div>
                      </TableCell>
                      <TableCell className="text-right">
                        <Button onClick={() => handleDownload(record.evidence_id, record.filename)} variant="secondary" size="sm" className="h-7 text-[10px] font-bold tracking-widest bg-primary/10 text-primary hover:bg-primary/20 border-none">
                          DOWNLOAD
                        </Button>
                      </TableCell>
                    </TableRow>
          ...
          <Card className="border-slate-200 shadow-md bg-white relative overflow-hidden">
            ...
            <CardHeader className="pb-4 border-b border-slate-100 bg-slate-50/50">
              <div className="flex items-center justify-between">
                ...
                <Badge variant="secondary" className="text-[9px] px-1.5 py-0 font-bold bg-primary/10 text-primary border-none">LIVE</Badge>
              </div>
            </CardHeader>
            <CardContent className="px-4 py-6">
              <div className="flex flex-col gap-3">
                {evidence.length === 0 ? (
                  ...
                  evidence.map((item, idx) => (
                    <div key={idx} className="flex flex-col gap-2 p-3 rounded-xl bg-slate-50 border border-slate-200 shadow-sm hover:border-primary/30 transition-all cursor-default group">
                      <div className="flex items-center justify-between">
                      ...
                      <div className="bg-slate-100 rounded-md p-2 border border-slate-200">
                        <p className="text-sm font-semibold text-emerald-600 break-all font-mono leading-tight">
                          {item.last_line}
                        </p>
                      </div>
                      <div className="flex items-center justify-between mt-1">
                        <Badge variant="outline" className="text-[8px] px-1 py-0 h-4 border-slate-300 text-slate-500 font-medium lowercase">
                          {item.filename}
                        </Badge>
                        <span className="text-[8px] text-slate-500 font-mono truncate max-w-[100px]" title={item.tx_id}>
              ...
              <Button variant="ghost" className="w-full mt-6 text-[10px] font-bold tracking-widest uppercase text-slate-500 hover:text-primary flex gap-2 hover:bg-slate-50 transition-all" onClick={() => fetchData(true)}>
                <RefreshCw className="h-3 w-3" /> Refresh Streams
              </Button>
            </CardContent>
          </Card>

          <Card className="bg-slate-800 border border-slate-700 shadow-lg relative overflow-hidden min-h-[140px] flex flex-col justify-center group">
            ...
            <CardContent className="p-6 relative z-10">
              ...
              <p className="text-slate-300 text-[10px] leading-relaxed mb-4 font-medium">
                All security telemetry is cryptographically signed and committed to the decentralized registry to prevent tamper-attacks.
              </p>
              <Button className="w-full bg-white/10 backdrop-blur-md hover:bg-white/20 text-white font-bold text-[10px] uppercase tracking-widest h-9 border border-white/20 shadow-lg transition-all">
                Network Config
              </Button>
            </CardContent>
          </Card>
